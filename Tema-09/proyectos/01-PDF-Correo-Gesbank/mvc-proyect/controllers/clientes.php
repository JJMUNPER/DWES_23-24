...
class Clientes extends Controller
{
    ...
    // Función para exportar datos de clientes a un archivo CSV
    public function exportar() {
        ...
        // Configurar los encabezados HTTP para indicar que el contenido es un archivo CSV
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="clientes.csv"');
    
        // Abrir un archivo de salida para escribir los datos de los clientes
        $ficheroExport = fopen('php://output', 'w');
    
        // Iterar sobre cada cliente y escribir sus datos en el archivo CSV
        foreach ($clientes as $cliente) {
            ...
            // Escribir cada fila de cliente en el archivo CSV
    ...
    // Función para importar datos de clientes desde un archivo CSV
    public function importar() {
        session_start();
    
        // Verificar autenticación y privilegios similares a la función exportar
        if (!isset($_SESSION['id'])) {
            $_SESSION['mensaje'] = "Usuario no autentificado";
            header("location:" . URL . "login");
            exit();
        ...
        // Verificar si la solicitud es POST y si se ha subido un archivo CSV
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["archivo_csv"]) && $_FILES["archivo_csv"]["error"] == UPLOAD_ERR_OK) {
            ...
            if ($handle !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
                    ...
                    // Validar si el email y el DNI son únicos
    ...
    // Función para generar un PDF con los datos de los clientes
    function pdf() {
        session_start();
        ...
        // Crear una instancia de la clase para generar el PDF
        $pdf = new pdfClientes();
